...
function styles() {
  return src(files.scss).pipe(sass())
...
//optimize and move images
function webp() {
  return src(files.webpSrc).pipe(imagewebp())
    .pipe(dest(files.imgDist));
}
...
function serve() {
  ...
  watch([files.templates, files.scss, files.imgSrc, files.jsSrc]).on("change", server.reload);
}

// Default Gulp task
exports.default = series(modernizrTask, parallel(html, styles, scripts, images), webp, svg, serve);
exports.build = series(modernizrTask, parallel(html, styles, scripts, imagesmin), webp, svg);
